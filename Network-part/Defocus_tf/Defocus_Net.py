...
class Defocus_Net():
    ...
    """
        ...
        Parameters:
            ...
            input_z - Magnification parameter added as a tensor --> Dimensions - [batch_size*z_dim]
    ...
    def build_graph(self):
        ...
        self.input_z = tf.placeholder(tf.float32, name="magnification", shape = [None,self.z_dim])

        x = self.inp
        label = self.blur
        m = self.input_z
        #for f_measure loss function
        sharp = x[:,:,:,:3]
        f = x[:,:,:,3:]
        ...
        self.gene_img = self.generator(x, m, reuse = False)
        ...
        """
            ...
            2.Focus Measure loss
            3.Adversarial loss
            4.GP_loss

            Generator loss     = Adversarial loss + 100*Content loss + 50*Focus measure loss 
        """    
        if self.mode == 'train':
            ...
            #Focus_measure loss
            self.loss_FM = fm_loss(sharp, f, self.gene_img)

            self.D_loss = d_loss_real + d_loss_fake + 10.0 * GP_loss
            self.G_loss = - d_loss_fake + 100.0 * self.content_loss + 50.0 * self.loss_FM
